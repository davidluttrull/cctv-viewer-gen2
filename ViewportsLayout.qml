        ...
        Repeater {
            ...
            delegate: Item {
                ...
                Item {
                    ...
                    // Debounced form of "maximized", used to pick the stream quality. Toggling
                    // maximize tears down and rebuilds the RTSP connection, so a burst of quick
                    // double-clicks would otherwise queue up several handshakes. Waiting for the
                    // state to settle collapses a burst into at most one stream change, and lets
                    // the maximize animation finish before the new stream starts loading.
                    // Deliberately not a binding: it is assigned only from the settle timer.
                    ...
                    readonly property url activeUrl: (maximizedSettled && String(d2.urlHigh) !== "") ? d2.urlHigh : d2.url
                    ...
                    Component.onCompleted: {
                        // Seed the settled state without waiting for the timer, so a layout that
                        // starts out maximized (a 1x1 preset) opens on the right stream instead of
                        // loading the low-quality one and immediately switching.
                        maximizedSettled = maximized;
                    }
                    onFocusChanged: {
                    ...
                    Timer {
                        ...
                        // Slightly longer than the 250ms maximize transition above, so the stream
                        // reload lands on a window that has stopped moving.
                        interval: 300

                        onTriggered: viewport.maximizedSettled = viewport.maximized
                    }
